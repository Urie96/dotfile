...
def cache-path [
  ...
  let tmp_dir = ($env.TMPDIR? | default "/tmp" | path join "yazi")
  mkdir $tmp_dir
  $tmp_dir | path join $"($md5).($ext)"
}
...
def with-cache [
  ...
  if ($env.DEBUG? == "1") or (not ($cache_path | path exists)) or (ls $cache_path | first | $in.size == 0B) {
...
def preview-ipynb [] {
  with-cache {
    if (exists-command jupytext) {
      jupytext --to markdown $env.filepath -o - | glow
    } else if (exists-command jupyter-nbconvert) {
...
def preview-apk [] {
  if not (cache-path | path exists) {
    let aapt = (
      (which aapt).0?.path?
      | default (glob ($env.HOME | path join "Library" "Android" "sdk" "build-tools" "**" "aapt")).0?
    )
    if $aapt == null {
      return ""
    }
...
def preview-lib [] {
  nm --extern-only $env.filepath | print
}
...
def preview-plist [] {
  if ("/usr/libexec/PlistBuddy" | path exists) {
    /usr/libexec/PlistBuddy -c print $env.filepath | bat -l nix | print
  } else {
    bat | print
  }
}
...
def get-mac-app-icon-path-from-plist [plistpath: string] {
  ...
  | (glob ($plistpath | path dirname | path join "Resources" "**" $"($in)*")).0?
  | if $in != null {
    let icon_path = $in
...
def preview-dmg [] {
  if not (cache-path | path exists) {
    ...
    if $dmg_info != null {
      $dmg_info | get -o mac_icon_path | default "" | if ($in | path exists) {
        let mac_app_path = $in
        with-cache --img --ext png {
          open -r $mac_app_path
        }
      }
      $dmg_info | get -o mac_app_info | default "" | do {
        let mac_app_info = $in
        with-cache {
          $mac_app_info
        }
...
def preview-woff [] {
  with-cache --img --ext png --nostdout {|cache_path|
    ...
    ^woff2otf $env.filepath $tmp_otf | ignore
...
def preview-woff2 [] {
  with-cache --img --ext png --nostdout {|cache_path|
    ...
    let tmp_woff2 = ($tmp_dir | path join "a.woff2")
    let tmp_ttf = ($tmp_dir | path join "a.ttf")
...
def preview-html [] {
  if (exists-command w3m) {
    w3m -dump -T text/html $env.filepath | print
  } else if (exists-command lynx) {
    lynx -dump -nolist -force_html -display_charset=utf-8 $env.filepath | print
  } else {
    bat $env.filepath | print
  }
}
