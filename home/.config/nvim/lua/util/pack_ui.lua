  ...
  local function finish_one(result)
    vim.schedule(function()
      ...
      if result then
        ...
        if result.unreleased then state.unreleased[result.name] = result.unreleased end
      ...
      if remaining == 0 then
        ...
        for name, commits in pairs(state.updates) do
          if #commits > 0 then state.expanded[name] = true end
        end
        render()
      end
    end)
        ...
        vim.system({ 'git', '-C', path, 'tag', '--list', '--sort=-version:refname' }, { text = true }, function(tag_res)
          ...
          local function after_released()
            resolve_remote_ref(path, function(ref)
              ...
              git_log(path, compare_from .. '..' .. ref, function(unreleased)
                if #unreleased > 0 then result.unreleased = unreleased end
          ...
          if is_newer and latest_tag then
            git_log(path, 'HEAD..' .. latest_tag, function(commits)
              ...
              if #commits > 0 then result.latest_ref = latest_tag end
...
local function setup_keymaps()
  ...
  -- Check for updates
  vim.keymap.set('n', 'C', M.check_updates, opts)
